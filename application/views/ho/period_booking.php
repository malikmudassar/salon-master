...
<div class="wrapper">
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-12">
                <h4 class="page-title">Period Booking: </h4>
            </div>
        </div>
        <!--Repeat Box-->
                    ...
                    <div class="col-lg-12">
                        <div class="card-box">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="staffcombo">Select Referring Staff:</lable>
                                        <select id="staffcombo" class="form-control"></select>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-8">
                                <div class="col-md-4">
                                    <div class="input-group">
                                        ...
                                        <input class="form-control numeric" readonly="readonly" name="advance_amount" id="advance_amount" onchange="updateadvances();" style="text-align: right;" value="0" />
                                    </div>
                                </div>
<!--                                <div class="col-md-2">
                                    <div class="form-group">
    ...
    function getpackagetypes() {
        $.ajax({
            type: 'POST',
            data:{param:'none'},
            url: "<?php echo base_url() . 'ho_controller/get_package_types'; ?>",
    ...
    function getpackagecategory() {
        ...
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url() . 'ho_controller/get_package_category'; ?>",
            data:{package_type_id: $("#packageselect :selected").val()},    
            success: function(response) {
                var data = $.parseJSON(response);
                var records=data.length;
                var totalprice = 0;
                chtml="<table id='visittable' class='table table-responsive'><thead><th>Category</th><th>Price</th><th>Day</th><th>Hour</th><th>Staff</th><th>Advance</th><th>Adv. Date</th><th>Mode</th><th>Inst#</th><th>Remarks</th><thead><tbody>";
                for (x = 0; x < data.length; x++) {
                    totalprice=totalprice+parseFloat(data[x]['Price']);
                    
                    chtml+="<tr><td id='"+ x +"_category'>" + data[x]['service_category']  + "</td>";
                    chtml+="<td id='"+ x +"_price'>" + data[x]['Price']  + "</td>";
                    chtml+="<td ><input style='width:120px !important;' id='"+ x +"_day' onchange='updateothers("+ x +")' class='form-control day' type='text' placeholder='mm/dd/yyyy'/></td>";
                    chtml+="<td ><input id='"+ x +"_hour' class='form-control' type='text' placeholder='hh:mm' style='width:80px !important;' /></td>";
                    chtml+="<td ><select id='"+ x +"_staff' business_id='" + data[x]['business_id'] + "' onchange='updateotherstaff("+ x +")' class='form-control staff'><option>Staff</option></select></td>";
                    chtml+="<td style='display:none;'>"+  data[x]['id_package_category'] +"</td>";
                    chtml+="<td><input style='width:100px !important;' id='" + x + "_advance' name='tdadvance' onkeyup='if(parseFloat($(this).val()) > " + parseFloat(data[x]['Price']) + "){ $(this).val("+data[x]['Price']+");}' onchange='updateadvances();' class='form-control numeric adv' type='number' value='0'/></td>";
                    chtml+="<td ><input style='width:120px !important;' id='"+ x +"_adv_date' class='form-control' type='text' placeholder='mm/dd/yyyy'/></td>";
                    
                    chtml+="<td ><select id='" + x + "_mode' class='form-control' type='text'><option value='cash'>Cash</option><option value='card'>Card</option><option value='check'>Check</option></select></td>";
                    chtml+="<td ><input style='width:80px !important;' id='"+ x +"_instrument' class='form-control' type='text'/></td>";
                    chtml+="<td ><input id='"+ x +"_remarks' class='form-control' type='text'/></td>";
                    chtml+="<td style='display:none'><input id='"+ x +"_businessid' class='form-control' type='text' value='"+ data[x]['business_id'] +"'/></td></tr>";
                }
                chtml+="<tfoot><th>Total:</th><th>" + parseFloat(totalprice) + "</th><th col=6></th></tfoot>";
                chtml+="</tbody></table>";
                ...
                for (x=0; x<records; x++){
                    ...
                    $("#"+x+"_adv_date").datepicker({autoclose:true,todayHighlight:true});
                    $("#"+x+"_adv_date").datepicker( "setDate", new Date());
...
//                      });
                    ...
                    $("#staffcombo > option").each(function() {
                        if($(this).attr('business_id')== $('#'+x+"_staff").attr('business_id')){
    ...
    function getstaff(){
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url() . 'ho_controller/get_active_staff'; ?>",
            data:{param:'none'},
            success: function(response) {
                var data = $.parseJSON(response);
                $("#staffcombo").append('<option></option>');
                for (x = 0; x < data.length; x++) {
                    $("#staffcombo").append('<option business_id='+data[x]['business_id']+' value='+data[x]['id_staff']+'>'+data[x]['staff_fullname']+'</option>');
                }
            }
        });
    }
        ...
        $.each(TableData, function(index, value){
            ...
            $.each(value, function(key, value){
               // console.log(key + ' > ' +value);
                if(key=="day" && value=="" || key=="hour" && value=="" || key=="staffid" && value=="Staff" || key=="staffname" && value==""){
                    filled=false;
                }
            });
    ...
    function create_new_visits(adv){
        ...
        $("#btncreatevisits").attr('disabled','disabled');
        $.ajax({
            type: 'POST',
            url: "<?php echo base_url() . 'ho_controller/create_booking'; ?>",
            data: {tabledata:TableData, 
                    ...
                    visit_color_type:visit_color_type,
                    advance:advance,
                    advance_amount:advance_amount,
                    advance_mode:advance_mode,
                    advance_inst:advance_inst,
                    visit_remarks:visit_remarks,
                    referring_staff: $("#staffcombo option:selected").text()
                },
            success: function(response) {
                res=response.split("|");
                //console.log(res[0]);
                if(res[0]=="success"){
                    window.open('<?php echo base_url(); ?>ho_print_booking/' + res[1]);
                    window.location.href='<?php echo base_url(); ?>ho_view';
                } else {
                    swal({
                        title: "Error!",
                        text: res[1],
    ...
    function storeOTblValues()
    {
        ...
        $('#visittable tr').each(function(row, tr) {
            TableData[row] = {
                ...
                , "day": $(tr).find('td:eq(2)>input').val()
                , "hour": $(tr).find('td:eq(3)>input').val()
                , "staffid": $(tr).find('td:eq(4)>select :selected').val()
                , "staffname": $(tr).find('td:eq(4)>select :selected').text()
                , "category_id": $(tr).find('td:eq(5)').text()
                , "advance": $(tr).find('td:eq(6)>input').val()
                , "advance_date": $(tr).find('td:eq(7)>input').val()
                , "mode": $(tr).find('td:eq(8)>select :selected').val()
                , "instrument": $(tr).find('td:eq(9)>input').val()
                , "remarks": $(tr).find('td:eq(10)>input').val()
                , "businessid": $(tr).find('td:eq(11)>input').val()
            }
        });
        TableData.shift();  // first row will be empty - so remove
        TableData.pop();  // first row will be empty - so remove
        return TableData;
    }
    ...
    function checkadvance(price, advbox){
        var inputbox = '#'+advbox;
        
        if(parseFloat($(inputbox).val()) > parseFloat(price)){
            $('#'+advbox).val(price);
        }
    ...
    function updateadvances(){
        ...
        $( ".adv" ).each( function( index, element ){
           // console.log( $( this ).val() );
            totaladvance=totaladvance+parseFloat($(this).val());
        });
        
        $("#advance_amount").val(totaladvance);
    }
