                            ...
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h4 class="modal-title"><strong>Add New Appointment</strong></h4>
                                    </div>
                                    <div class="modal-body">
                                                ...
                                                <div class="card-box">
                                                    ...
                                                    <div class="row p-20">
                                                        <div class="col-sm-8 col-sm-offset-2">
                                                            ...
                                                            <button type="button" onclick="openVisit(0);" class="btn btn-pink btn waves-effect w-md waves-info m-b-5">Appointments</button>
...
<script>
    ...
    function createEvents(service_id, service_duration, service_color, visitid, appointment_sid){
        ...
        $.ajax({
            type: "POST",
            url: "Appointment_controller/updateAppointmentTime",
            data: "visitid=" + visitid + "&service_id=" + service_id + "&start=" + newstart + "&end=" + newend + "&appointment_sid=" + appointment_sid,
            success: function(data){
                if (data === "success") {
                    return true;
                } else{
    ...
    function updatevisit(){
        ...
        var appointment_id = $("#visit-id-1").val() != "" ? $("#visit-id-1").val() : 0;
        ...
        var dataInfo = 'service_id=' +service_id+ '&service_name=' +service_name+ '&staff_id=' +staff_id+ '&staff_name=' +staff_name+ '&customer_id=' +customer_id+ '&appointment_id=' +appointment_id+ '&product_ids=' +product_ids+ '&product_names=' +product_names;

        $.ajax({
            type: "POST",
            url: "Appointment_controller/addappointments",
            data: dataInfo,
            success: function(data){
                ...
                if (result[0] === "success") {
                    ...
                    appointment_sid = result[2];

                    createEvents(service_id, service_duration, service_color, result[1], appointment_sid);
    ...
    function fillappointment(visitid,qtype){
        var myurl
        if(qtype == 1){
            myurl= 'Appointment_controller/getAppointmentbyid/1';
        } else {
            myurl= 'Appointment_controller/getAppointmentbyid';
        }

        $.ajax({
            type: 'POST',
            url: myurl,
            data:{id_customer_appointment: visitid},
            dataType: "json",
            cache: false,
            async: true,
            success: function(data) {
                //console.log(data);
                if(qtype == 1){
                    $("#appointment-id-1").val('');
                } else {
                    $("#appointment-id-1").val(data[0]['id_customer_appointments']);
                }
                $("#appointment-customer-id").val(data[0]['customer_id']);
                $("#appointment-customer-name").val(data[0]['customer_name']);
                $("#appointment-customer-cell").val(data[0]['customer_cell']);
                $("#appointment-customer-email").val(data[0]['customer_email']);
    ...
    function checkopenappointment(cid){
        $.ajax({
           type: 'POST',
           url: 'Appointment_controller/getAppointmentbyCid',
           data: {customerid: cid},
           dataType: "json",
           cache: false,
           async: true,
           success: function(data) {
                ...
                if(data.length>0){
                    clearvisit();
                    fillvisit(data[0]['id_customer_appointments']);
                }   
           }
       });
   }
    ...
    function openVisit(visitid, cid) {
        ...
        if(visitid !== 0){
            ...
            checkopenappointment($("#txt-customer-id").val());
            $("#divmain").slideUp();
            $("#divvisit").slideDown();
        }
        //getlast4visits();
...
//        });
        ...
        var calendar = $('#calendar').fullCalendar({
            ...
            resourceRender: function(resourceObj, labelTds, bodyTds) {
                $.ajax({
                    url: '<?php echo base_url().'Appointment_controller/getStaffImages'; ?>',
            ...
            eventClick: function(calEvent, jsEvent, view) {
                ...
                $.ajax({
                    url: 'Appointment_controller/getvisitdetails',
                    data: 'appointment_service_id=' +calEvent.id,
                    type: 'POST',
                    success: function(response){
                        ...
                        var startdt = data[0].appointment_service_start;
                        startdt = startdt.split("T");

                        var startdate = startdt[0].replace(/-/g, '/');
                        var starttime = startdt[1];

                        var enddt = data[0].appointment_service_end;
                        enddt = enddt.split("T");
                        ...
                        $('#visit-id').val(data[0].customer_appointment_id);
            ...
            eventResize: function(event, delta, revertFunc) {
                ...
                $.ajax({
                    url: 'Appointment_controller/updatevisit',
                    data: 'id_appointment_services=' +event.id+ '&staff_id=' +event.resourceId+ '&start=' +event.start.format()+ '&end=' +event.end.format(),
                    type: 'POST',
                    success: function(response){
