<div class="wrapper">
                                        ...
                                        <div class="col-lg-12"> 
                                            <select id="selectreport" onchange="reportchanged();" class="form-control">
                                                ...
                                                <optgroup label='Appointments'>
                                                    <option value="cancelled_visits">Canceled Appointments</option>
                                                    ...
                                                    <option value="sharedcustomers">Shared Customers</option>
                                                 </optgroup>   
    ...
    <!--                                                <option value="ssales">Services Sale</option>-->
                                                    ...
                                                    <option value="voucher">Vouchers Sold</option>
                                                    <option value="cash_register">Cash Register</option>
                                                    <option value="payment_breakup">Payment Breakup</option>
                                                    <option value="payment_breakup_services">Payment Breakup Services</option>
                                                    <option value="payment_breakup_sale">Payment Breakup Retail</option>
                                                    <option value="receivables">Receivables For Open Visits</option>
                                                    <option value="advance_collected">Advance Collected</option>
                                                    <option value="recoverinvoices">Open Recovery Invoices</option>
                                                    <option value="loyaltyredemption">Loyalty Redemption</option>
                                                </optgroup>
                                                <?php } ?>    
                                                <optgroup label='Retail & Stock'>
                                                    <option value="pusages">Product Usage Summary</option>
                                                    <option value="stock_status">Stock Status</option>
                                                    <option value="stock_status_brand">Stock Status By Brand</option>
                                                    <option value="staff_retail_sale_summary">Staff Product Sale</option>                                                
                                                    <option value="dispatch">Day Wise Product Dispatches</option>
                                                    <option value="dispatch_details">Dispatch Note Details</option>
                                                </optgroup>
                                                <?php if($this->session->userdata('role')!=="Store Manager"){?>    
                                                <optgroup label='Staff'>
                                                    <option value="attendance">Attendance</option>
                                                    <option value="commissions">Service Commissions</option>
                                                    <option value="retailcommissions">Retail Commissions</option>
...
//                    },
            ...
            function msales() {
                ...
                $.ajax({
                    ...
                    success: function(alldata) {
                        ...
                        var hhtml = "<tr><th>Month</th><th>Year</th><th>Sub Total</th><th class='text-danger'>Discount</th><th>Gross</th><th>Taxes</th><th class='text-success'>Net Amount</th><th class='text-info'>Invoice</th>,<th>Advance</th>,<th>Voucher</th>,<th>Total</th></tr>";
                        ...
                        for (x = 0; x < data.length; x++) {
                            ...
                            var none = true;
                            for (y = 0; y < advance.length; y++) {
                               if(advance[y]['Month']==data[x]['Month'] && advance[y]['Year']==data[x]['Year']){
                                    mhtml += '<td class="text-info" style="font-weight:bold;">' + advance[y]['TotalAdvance'] + '</td>';
                                    totalrec=totalrec+parseFloat(advance[y]['TotalAdvance']);
                                    none=false;
                                } 
                            }
                            if(none==true){
                                mhtml += '<td class="text-info" style="font-weight:bold;">0</td>';
                            }
                            
                            var none = true;
                            for (y = 0; y < voucher.length; y++) {
                               if(voucher[y]['Month']==data[x]['Month'] && voucher[y]['Year']==data[x]['Year']){
                                    mhtml += '<td class="text-info" style="font-weight:bold;">' + voucher[y]['TotalVoucher'] + '</td>';
                                    totalrec=totalrec+parseFloat(voucher[y]['TotalVoucher']);
                                    none=false;
                                } 
                            }
                            if(none==true){
                            ...
                            mhtml += '<td class="text-danger" style="font-weight:bold;">' + totalrec + '</td>';
                            mhtml += '</tr>';
            ...
            function sscategory(){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var totalpaid=0;
                        var hhtml = "<tr><th>Type</th><th>Category</th><th>Count</th><th>Price</th><th>Service Discount</th><th>Invoice Discount</th><th class='text-success'>Discounted Price</th><th class='text-info'>Paid</th></tr>"; //<th class='text-danger'>Balance</th></tr>";
...
//                            totalrecovery=totalrecovery+parseFloat(data[x]['paid']);}
                            ...
                            mhtml += '<td class="text-danger">' + data[x]['discount'] + '</td>';
                            mhtml += '<td class="text-danger">' + (parseFloat(data[x]['price'])-parseFloat(data[x]['discounted_price'])) + '</td>';
                            mhtml += '<td class="text-success" style="font-weight:bold;">' + data[x]['discounted_price'] + '</td>';
                            mhtml += '<td class="text-info" style="font-weight:bold;">' + data[x]['paid'] + '</td>';
//                            mhtml += '<td class="text-danger" style="font-weight:bold;">' + data[x]['balance'] + '</td>';
                            mhtml += '</tr>';
                            totalpaid = totalpaid + parseFloat(data[x]['paid']);
                        ...
                        totalpaid=totalpaid.toFixed(2);
                        var fhtml="<tr><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th>"+totalpaid.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",")+"</th></tr>"
                        //console.log(mhtml);
                        $("#tblreport").dataTable().fnDestroy();
                        $("#tblreport thead").html('');
                        $("#tblreport tbody").html('');
    ...
    //                            }
            ...
            function staffperf() {
                ...
                $.ajax({
                    ...
                    async: true,
                    success: function(data) {
                        var hhtml = "<tr><th>Staff</th><th>Type</th><th>Category</th><th>Service</th><th>Count</th><th>Price</th><th>Discount</th><th>Payable</th><th>Paid</th></tr>"
                        ...
                        for (var x = 0; x < data.length; x++) {
                            ...
                            mhtml += '<td>' + data[x]['service_count'] + '</td>';
                            mhtml += '<td>' + data[x]['price'] + '</td>';
                            mhtml += '<td class="text-danger">' + data[x]['discount_sum'] + '</td>';
                            mhtml += '<td class="text-success">' + data[x]['price_sum'] + '</td>';
                            mhtml += '<td class="text-info" style="font-weight:bold">' + data[x]['paid'] + '</td>';
                            mhtml += '</tr>';
