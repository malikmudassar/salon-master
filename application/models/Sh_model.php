...
///dashboard///
    function get_business_sale_year_month($month, $year){
        ...
        $query = $this->db->get('sh_business_sales');
    ...
    function add_this_month_year_sale($data){
        $this->db->insert('sh_business_sales', $data);
    ...
    function get_workweek(){
        ...
        ifnull(paid_amount,0) 'Revenue', ifnull(balance,0) 'Balance', 
	 ifnull(paid_amount,0)
        ...
        left join (
                select date_format(invoice_date, '%Y-%m-%d') invoice_date, count(customer_name) as customers,
        ...
        left join (
				...
				count(service_name) as services 
    ...
    function get_workmonth(){
        ...
        left join (
                select date_format(invoice_date, '%Y-%m-%d') invoice_date, count(customer_name) 'customers',
                ifnull(sum(paid_amount),0) paid_amount, ifnull(sum(balance),0) as balance
                from invoice 
                where invoice_status = 'valid'
                and invoice_seq > 0 
    ...
    function get_todaysale() {
        ...
        $sql="SELECT Date_Format(invoice_date, '%Y-%m-%d') 'saleon', 
    ...
    function top_4_staff(){
        ...
        $year=date('Y', strtotime($today));
                ...
                staff.staff_image, 
                count(id_invoice_staff)  as 'Services', 
                format(sum(paid), 2) as 'Paid', 
                ifnull(a.Recovery,0) 'Recovery', 
                format(ifnull(sum(paid) + ifnull(a.total,0),0),2) as 'Amount',
                ifnull(sum(invoice_details.paid) + ifnull(a.total,0),0)
                FROM invoice_staff 
                Join invoice_details on invoice_details.id_invoice_details=invoice_staff.invoice_detail_id
                JOIN invoice ON invoice.id_invoice = `invoice_staff`.`invoice_id` 
                JOIN staff ON staff.id_staff = invoice_staff.staff_id 
                left join (
                        SELECT id_staff, format(sum(ifnull(invoice_details.paid,0)), 2) as 'Recovery', 
                        sum(ifnull(invoice_details.paid,0)) as 'total',
                        count(id_invoice_staff) as 'rec_count'
                        FROM `invoice_staff` 
                        Join invoice_details on invoice_details.id_invoice_details=invoice_staff.invoice_detail_id
                        JOIN `invoice` ON `invoice`.`id_invoice` = `invoice_staff`.`invoice_id` 
                        JOIN staff ON staff.id_staff = invoice_staff.staff_id 
                ...
                AND invoice_seq > 0 
                AND invoice.business_id=".$this->session->userdata('businessid')."
                GROUP BY invoice_staff.staff_id, staff.staff_image
                ORDER BY 8 desc";
