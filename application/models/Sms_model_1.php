...
class Sms_model extends CI_Model {
    ...
    function send_sms($msg, $phone, $debug=false, $using="SMSReminder", $echo=true)
    {
        if ($msg == '' || $phone == '' || strlen($phone)!==11){
            ...
            if($sms_cred){
               ...
               // $response = $this->url_get_contents($urltouse);
                ...
                $updated = $this->update_smslog($response, $using, $msg, $echo);
...
//        return $output;
//    }
    function update_smslog($response, $using="SMSReminder", $msg, $echo=true){
        ...
        if($response !== 'SMS Service is not setup!' ){
           //echo $response;
            ...
            $xml= json_decode($response);
            $responsestatus = $xml->response->status;
            
            if($responsestatus!=='Error'){
                $this->db->set('errorno', $xml->response->errorno);
                $this->db->set('status', $xml->response->description);
                ...
                if($echo == true){
                    ...
                    echo($xml->response->description).'<br/>';
                    echo( $msg ).'<br/>';
