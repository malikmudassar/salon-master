...
class Purchaseorder_controller extends CI_Controller {
    ...
    public function purchase_order_list() {
        if($this->session->userdata('role')=='' || $this->session->userdata('role')=='Sh-Users'){
            ...
            $data['subnav'] = 'purchaseorder';

            $data['business'] = $this->business_model->getbusinessdetails();
            $data['businesses'] = $this->business_model->get_all_businesses();
            //$data['purchase_orders'] = $this->purchaseorder_model->getPurchase_order_list();
            $data['suppliers'] = $this->supplier_model->get_suppliers($this->session->userdata('businessid'));
            //$data['products'] = $this->product_model->get_product_list('professional');
    ...
    public function purchase_order_list_get() {
        $data = $this->purchaseorder_model->getPurchase_order_list();
    ...
    public function grn_print($grn_id = 0) {
        if($this->session->userdata('role')=='' || $this->session->userdata('role')=='Sh-Users'){
            $this->load->view('login_view');
        } else {
            //storemanager,hr,reception...user serial
            ...
            //$data['purchaseorder'] = $this->purchaseorder_model->get_purchase_order_by_id($grn_id);


            $this->load->view('includes/header', $data);
            $this->load->view('good_recieved_note_print');
            $this->load->view('includes/footer');
        }
    }
